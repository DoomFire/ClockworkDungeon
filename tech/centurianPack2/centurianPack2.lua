...
function leverState.clockDebug(text)
	...
	local npcids = world.npcQuery(tech.position(),50, {order = "nearest"})
	
	if npcids and npcids[1] then	
		local testvar=npcids[1]
		world.logInfo("test: "..tostring(testvar))
		world.callScriptedEntity(testvar, "entity.say",text)
